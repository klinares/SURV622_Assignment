...
To prepare the data for analysis, a comprehensive preprocessing pipline were conducted in Pytho 3.
...
From this cleaned dataset, 400 uniqe comments without replacement were randomly sampled for manual annotation.
Four graudate students independently coded and labeled each comment with one of three possible stances:favor, opopose, or neutral toward DOGE’s approach to federal workforce reductions.
...
The feature set used in model training and evluation consisted of two primary categories: metadata features and textual features.
...
Textual features were derived from each comment and represented using two distinct approaches: unigram term frequency and term freuqency-inverse document frequency (TF-IDF).
...
Two datasets were constructed for classification: one combining the metadata with unigram features, and another with metadat and TF-IDF features.
Each dataset was divded into 80% training data and 20% test data.
...
All models were trained using default hyperparameter confiurations.
...
Recall was also independently assessed to evaluate the proportion of true postive cases accurately identified by each model.
